...
import ExportFormattedTable from "../../../../components/ExportFormattedTable";
...
const TransactionPage = () => {
  ...
  const [transactionStatus] = useState([
    { id: "0", label: "All Received Status", value: "All" },
  ...
  const handleSearch = useCallback(
    debounce(() => {
      ...
      const filters = [searchFilter, searchStatus, receivedMode, orderBy, pagination]
        .filter(Boolean)
        .join("&");
  ...
  const handleGetTransaction = async (filter: string, orgFilters: Array<string>): Promise<void> => {
    ...
    try {
      ...
      console.log("getTransactionList", response)
      if (response && Array.isArray(response.data)) {
        const formattedData: Transaction[] = response.data.map((item: any) => ({
          isAnonymizePurpose: item.isAnonymizePurpose,
          isDeletedProfile: item.isDeletedProfile,
  ...
  const getOrganization = (org: any) => {
    ...
    var Orgs: string[] = [];
    Orgs.push("00000000-0000-0000-0000-000000000000");
  ...
  //   return dateString ? dayjs(dateString).format("DD/MM/YY HH:mm") : "-";
  // };
  ...
  const columns = useMemo(() => [
    {
      ...
      Cell: ({ row }: any) => <span className="text-base font-semibold text-primary-blue cursor-pointer" onClick={() => navigate("details/" + row.original.transactionId)}>{row.original.transactionId}</span>,
    },
    {
    ...
    },
    {
      ...
      Cell: ({ row }: any) => {
        if (row.original.isAnonymizePurpose === true || row.original.isDeletedProfile === true)
          return <span className="text-base font-normal">{row.original.profileIdentifier}</span>
        return <span className="text-base font-normal text-primary-blue cursor-pointer" onClick={() => navigate("/consent/data-subject/view/" + row.original.profileIdentifierId + "/information")}>{row.original.profileIdentifier}</span>
      }
    },
    {
      ...
      // <span className="text-base font-semibold text-primary-blue cursor-pointer" onClick={() => navigate("/consent/purpose/standard-purpose/view-spurpose/" + row.original.standardPurposeId)}>{row.original.standardPurposeName}</span>,
    },
    {
      ...
      Cell: ({ row }: any) => (
        <span className="flex justify-center items-center w-full">
          <Tag size="sm" minHeight="1.625rem" className="text-primary-blue font-medium px-3 py-1 rounded-md text-base bg-blue-100 max-w-max">
            Version {row.original.standardPurposeVersion ? row.original.standardPurposeVersion : "-"}
          </Tag>
        </span>
      ),
    },
    {
      ...
      Cell: ({ row }: any) => <span className="text-base font-semibold text-primary-blue cursor-pointer" onClick={() => navigate("/consent/receipts/details/" + row.original.receiptId)}>{row.original.receiptId}</span>,
    },
    {
